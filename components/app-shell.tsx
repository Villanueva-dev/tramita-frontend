...
export function AppShell({
  ...
  useEffect(() => {
    if (authStatus === 'unauthenticated' || (!isAuthenticated && authStatus !== 'loading')) {
      router.replace('/')
    }
  ...
  async function handleLogout() {
    // La navegación ocurre solo después de invalidar la sesión en el backend.
    setLoggingOut(true)
    setLogoutError('')
    try {
      await logout()
      ...
      // Conserva la pantalla y explica el fallo para permitir reintentar.
  ...
  function renderSidebarContent(onClick?: () => void) {
    return (
  ...
  return (
    <div className="flex min-h-screen bg-background">
      ...
      <div className="flex flex-1 flex-col lg:pl-64">
        {logoutError && (
          <p role="alert" className="border-b border-destructive/30 bg-destructive/10 px-4 py-2 text-sm text-destructive lg:px-8">
            {logoutError}
          </p>
        )}
